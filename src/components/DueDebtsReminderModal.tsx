...
// Modal shows once per mount (i.e. once per login). No sessionStorage — that
// would persist across logout/login in the same tab and block re-display.
...
export function DueDebtsReminderModal() {
  ...
  // Open once per mount (i.e. once per login) when debts have loaded and
  // there's something to remind. If the user adds a new overdue/today debt
  // mid-session, the modal still pops up the first time items becomes non-empty.
  useEffect(() => {
